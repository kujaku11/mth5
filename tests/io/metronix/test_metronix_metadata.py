...
class TestMetronixFileNameMetadata(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.obj = MetronixFileNameMetadata()
...
class TestMetronixJSONMagnetic(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.fn = metronix_data_path / "Run_008" / "084_ADU-07e_C002_THx_512Hz.json"
        with open(cls.fn, "w") as fid:
            json.dump(cls.magnetic_dict, fid)

        cls.magnetic = MetronixChannelJSON(cls.fn)

        cls.expected_magnetic_metadata = Magnetic(
            **OrderedDict(
                [
                    ("channel_number", 2),
                    ("component", "hx"),
        ...
        cls.expected_fap_dict = OrderedDict(
            [
                (
                    "amplitudes",
                    np.array(cls.magnetic_dict["sensor_calibration"]["a"]),
                ),
                ("calibration_date", "2006-12-01T11:23:02+00:00"),
                (
                    "frequencies",
                    np.array(cls.magnetic_dict["sensor_calibration"]["f"]),
                ),
                ("gain", 1.0),
                ("instrument_type", ""),
                ("name", "mfs-06_chopper_1"),
                (
                    "phases",
                    np.deg2rad(np.array(cls.magnetic_dict["sensor_calibration"]["p"])),
                ),
        ...
        cls.expected_fap = FrequencyResponseTableFilter(**cls.expected_fap_dict)
    ...
    def test_sample_rate(self):
        self.assertEqual(512.0, self.magnetic.sample_rate)
...
class TestMetronixJSONElectric(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.fn = metronix_data_path / "Run_008" / "084_ADU-07e_C000_TEx_128Hz.json"
        with open(cls.fn, "w") as fid:
            json.dump(cls.electric_dict, fid)

        cls.expected_electric_metadata = Electric(
            **OrderedDict(
                [
                    ("channel_number", 0),
                    ("component", "ex"),
        ...
        cls.electric = MetronixChannelJSON(cls.fn)
