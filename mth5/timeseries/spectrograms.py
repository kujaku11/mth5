"""
This will be an xarray accessor for creating spectrograms of time series data

There are various ways to calculate the spectrogram to focus energy and reduce noise.

Included here are time-frequency analysis tools translated into python during my PhD in 2012.
...
def reassigned_stft(
    ...
    half_index = int(nfbins / 2)
    return_flst = np.fft.fftfreq(nfbins, 1.0 / df)[0:half_index]
    ...
    for nn in range(nt):
        for kk in range(half_index):
            if abs(spec[kk, nn]) > threshold:
                ...
                khat = int(
                    np.remainder(
                        np.remainder(khat - 1, half_index) + half_index, half_index
                    )
                )
                # reassign energy
