...
for line in lines[170:]:
    if '\section{' in line:
        ...
        entries[key] = []
    if '\entry' in line:
        entries[key].append(line.strip().replace('\\entry{', '').replace('\_', '_').replace('}{', ',').replace('\\True', 'True').replace('\\False', 'False').replace('}', '').split(','))
...
for key, value in entries.items():
    ...
    with open(r"c:\Users\peaco\Documents\{0}_metadata_table.rst".format(key), 'w') as fid:
        fid.write('\n'.join(lines))    